...
class SupertonicTTS(private val context: Context) {
    ...
    fun generateSpeech(
        ...
        for (chunk in chunks) {
            val audio = generateChunk(chunk, voiceStyle, language, speed, totalSteps, actualSeed)
            allAudio.add(audio)
    ...
    private fun generateChunk(
        ...
        // Process text with language
        val (textIds, textMask) = textProcessor.processText(text, language)
